          ...
          <div class="col-sm-12">
              ...
              <div id="titleH{{$helpDataN->id}}">
               ...
               <div id="form{{$helpDataN->id}}">
                  ...
                  <form action="/admin/faq/view/post/{{$helpDataN->id}}" method="post">
                    {{ csrf_field() }}
                      Question <br>
                      <textarea name="question" id="" cols="100%" rows="3" value="" class="form-control" required >{{$helpDataN->question}}</textarea>
                      Your Answer<br>
                      <textarea name="answer" id="" cols="100%" rows="3" class="form-control" required >{{$helpDataN->answer}}</textarea>
                      <input type="hidden" name="approved" id="" value="Y"><br>
                      <input type="submit" value="Post" name="save" class="btn btn-primary">
                      <a href="/admin/faq/remove/{{$helpDataN->id}}" class="btn btn-danger" onclick="return confirm('Question will be removed! Are You Sure?');">Remove</a>
            ...
            @if (count($helpData)>0)
               @foreach ($helpData as $helpData)
                   ...
                   <span><a href="/admin/faq/remove/{{$helpData->id}}" style="color:red;" onclick="return confirm('Question will be removed! Are You Sure?');">Remove</a></span>
               @endforeach
               @else
                Not found
               @endif
               ...
               <div id="form2">
                  <form action="/admin/faq/view/post/" method="post">
                    {{ csrf_field() }}
                      Question <br>
                      <textarea name="question" id="" cols="100%" rows="3" value="" class="form-control" required></textarea>
                      Your Answer<br>
                      <textarea name="answer" id="" cols="100%" rows="3" class="form-control" required></textarea>
                      <input type="hidden" name="approved" id="" value="Y"><br>
                      <input type="submit" value="Post" name="save" class="btn btn-primary">
