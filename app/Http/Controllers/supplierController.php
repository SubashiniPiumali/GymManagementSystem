...
class supplierController extends Controller
{
    ...
    protected function viewmyorders($id){ //id.>> orderRefNo
       ...
       $reservedData=DB::table('orders')->join('reserves','reserves.order_Id','=','orders.id')
       ->join('productimages','productimages.id','=','orders.product_id')
       ->select('productimagename','image','quantity','reserves.id as r_id','reserved_quantity','order_Id','price','reserved_quantity','orders.created_at as o_created_at','reserved_price')
       ->where('supplier_Id','=',$supplier_Id)->where('reserves.order_refNo','=',$id)->get();
       ...
       $supplierData=DB::select("select * from suppliers where supplierId ='$supplier_Id' ");
        ...
        $customerData=DB::table('orders')->join('customers','customers.customerId','=','orders.customer_id')
        ->select('companyname')->where('orders.order_number','=',$id)->get();
        ...
        ->with('id',$id)->with('customerData',$customerData);
    ...
    protected function viewOrderslist(){
        ...
        //$reserves=DB::select("select* from reserves where supplier_Id='$supplier_Id'");
        $reserves=DB::table('reserves')->join('orders','orders.id','=','reserves.order_Id')->join('customers','customers.customerId','=','orders.customer_id')->select('reserves.id','order_refNo','reserved_at','companyname')->where('reserves.supplier_Id','=',$supplier_Id)->get();
...
$image=addslashes($_FILES['image']['tmp_name']);
    ...
    protected function subscriptionslist(){
        $id=Auth::user()->id;
        $supplierData=DB::select("select * from suppliers where supplierId = '$id'");
       
        $subscribeData=DB::table('subscribes')->join('customers','customers.customerId','=','subscribes.customerId')
        ->select('subscribes.id','companyname')->where('subscribes.supplierId','=',$id)->get();
     
        return view('supplierpages.subscriptionslist')->with('supplierData',$supplierData)
        ->with('subscribeData',$subscribeData);
    }

    protected function viewoffpay(){

        $id=Auth::user()->id;
        $supplierData=DB::select("select * from suppliers where supplierId = '$id'");

        //$paymentData=DB::select("select * from payments where supplierId = '$id'");
        $paymentData=DB::table('payments')->join('customers','customers.customerId','=','payments.customerId')
        ->select('companyname','slip_image')->where('payments.supplierId','=',$id)->get();
        //return $supplierData;
        return view('supplierpages.offlinepay')->with('supplierData',$supplierData)->with('paymentData',$paymentData);
