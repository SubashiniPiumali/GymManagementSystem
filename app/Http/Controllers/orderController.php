...
class orderController extends Controller
{
 ...
 foreach($students as $stud){
   ...
   foreach($ord as $or){
     ...
     //app('App\Http\Controllers\subscribeController')->sendSMS();
    ...
    public function viewreservedonesupplier(Request $request,$supplier_Id){
        $data=Auth::user()->id;
     ...
     $orderef=$request->orderef;
      ...
      $onlysupplierreserved = DB::table('reserves')->join('orders','orders.id','=','reserves.order_id')->join('productimages','productimages.id','=','orders.product_id')->select('reserved_quantity','reserved_price','product_id','productimagename','image')->where('reserves.customer_id','=',$data)->where('reserves.supplier_Id','=', $supid)->get();
              ...
              $displayname =DB::table('suppliers')->select('suppliers.id','image','name','address','phoneNo','email')->where('supplierId','=', $supid)->get();
             //new
              ...
              return view('customer_order.onesuppliersorders')->with('onlysupplierreserved',$onlysupplierreserved)->with('displayname',$displayname)
              ->with('customerdetailes',$customerdetailes)->with('totalprice',$totalprice)->with('supplierId',$supid);
...
public function makePayment(Request $request){
    ...
    $slip_image=addslashes($_FILES['slip_image']['tmp_name']);
    $slip_image=file_get_contents($slip_image);
    $slip_image=base64_encode($slip_image);
        
    $paymentData->slip_image =$slip_image;
    $paymentData->supplierId =$request->supplierId;
    $paymentData->customerId =$customerId;
    $supplier_Id=$request->supplierId;
    $paymentData->save();
    return  redirect("/show")->with('success','Payment Successfully...');
    return  redirect("/viewreservedonesupplier/$supplier_Id")->with('message','Payment Successfully...');
...
public function sendSMS(){
   ...
   $customerId=Auth::user()->id;
   $result = DB::select("select phoneNo from subscribes where customerId='$customerId'");
   ...
   foreach($result as $r){
    ...
    //echo $r->phoneNo;
        ...
        'to'   => '94' . $r->phoneNo,
        'from' => 'CeylonCart',
        'text' => $customerData->companyname.' has placed a new order.'
        ]); 
    ...
    return "Order Sucess, Sent Messages to the subscribers";
   //return $result;
}
